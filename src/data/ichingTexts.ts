import { hexagramNames } from "./hexagrams";

// 64卦完整版：二进制 key 按“自下而上”记录六爻，1=阳，0=阴。
// 说明：当前结构只保留每卦六条爻辞；乾卦“用九”、坤卦“用六”未并入 yaoci。
export const ichingTexts: Record<string, { guaci: string; yaoci: string[] }> = {
  "111111": {
    guaci: "乾：元，亨，利，贞。",
    yaoci: [
      "初九：潜龙，勿用。",
...
export function getGuaci(binary: string): string {
  if (ichingTexts[binary]) {
    return ichingTexts[binary].guaci;
  }
  const name = hexagramNames[binary] || "未知";
  return `${name}：此卦暂无内置卦辞，请参考《易经》原典。`;
}

export function getYaoci(binary: string, index: number): string {
  if (ichingTexts[binary] && ichingTexts[binary].yaoci[index]) {
    return ichingTexts[binary].yaoci[index];
  }
  const isYang = binary[index] === '1';
  const yinYangChar = isYang ? "九" : "六";
  const positionChar = index === 5 ? "上" : index === 0 ? "初" : ["二", "三", "四", "五"][index];
  const label = index === 0 || index === 5 ? `${positionChar}${yinYangChar}` : `${yinYangChar}${positionChar}`;
  return `${label}：此爻暂无内置爻辞，请参考《易经》原典。`;
}
